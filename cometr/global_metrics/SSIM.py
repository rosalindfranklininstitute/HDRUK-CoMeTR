...
class SSIM(Metric):
    ...
    def gaussian_blur_3d(
        ...
        def gaussian(
                sz: int,
                sg: float,
                dev: Union[int, str] = "cpu"
        ) -> Tensor:
            g = torch.exp(-torch.pow(torch.arange((1 - sz) / 2, (1 + sz) / 2, step=1, device=dev) / sg, 2) / 2)
            return (g / g.sum()).unsqueeze(dim=0)

        if self.kernel_3d is None:
            kernel_1d = gaussian(self.kernel_size[0], self.sigma[0], self.device)
            kernel_2d = torch.mul(torch.transpose(kernel_1d, 0, 1), gaussian(self.kernel_size[1], self.sigma[1], self.device))
            self.kernel_3d = torch.mul(
                kernel_2d.unsqueeze(-1).repeat(1, 1, self.kernel_size[2]),
                gaussian(self.kernel_size[2], self.sigma[2], self.device).expand(self.kernel_size[0], self.kernel_size[1], self.kernel_size[2])
            )

        height_pad = (self.kernel_size[0] - 1) // 2
        width_pad = (self.kernel_size[1] - 1) // 2
        depth_pad = (self.kernel_size[2] - 1) // 2
        inp = F.pad(inp, (height_pad, height_pad, width_pad, width_pad, depth_pad, depth_pad), mode="reflect")
        return F.conv3d(
            inp,
            self.kernel_3d.expand(inp.size(1), 1, self.kernel_size[0], self.kernel_size[1], self.kernel_size[2]),
            groups=inp.size(1)
        )
    ...
    def mu_sq(
        ...
        if A.dim() != 5:
            raise ValueError("Data from the input filename has to 3D (HxWXD) or 5D (BxCxHxWxD)")
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name + "_mu_sq.h5", file_key)).to(self.device)
        if B.dim() == 3:
    ...
    def co(
        ...
        # This is mu1
        A = torch.from_numpy(Metric.load_file(dir_path + "/" + name2 + "_mu.h5", self.file1_key)).to(self.device)
        if A.dim() == 3:
            A = A.expand(1, 1, A.size(0), A.size(1), A.size(2))
        if A.dim() != 5:
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name2 + "_mu.h5", self.file2_key)).to(self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
            nm = dir_path + "/" + name2 + "_mu.h5"
        ...
        A = torch.from_numpy(Metric.load_file(self.file1, self.file1_key)).to(self.device)
        if A.dim() == 3:
            A = A.expand(1, 1, A.size(0), A.size(1), A.size(2))
        if A.dim() != 5:
            raise ValueError(f"Data in the {self.file1} has to 3D (HxWXD) or 5D (BxCxHxWxD)")

        B = torch.from_numpy(Metric.load_file(self.file2, self.file2_key)).to(self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
            raise ValueError(f"Data in the {self.file2} has to 3D (HxWXD) or 5D (BxCxHxWxD)")

        A = A * B

        A = self.gaussian_blur_3d(A)

        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name1 + "_" + name2 + "_mu1_mu2.h5", self.file1_key)).to(self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
    ...
    def metric_calc(self, file1_data: np.ndarray, file2_data: np.ndarray) -> float:
        ...
        A = torch.from_numpy(Metric.load_file(dir_path + "/" + name1 + "_sigma_sq.h5", self.file1_key)).to(self.device)
        if A.dim() == 3:
            A = A.expand(1, 1, A.size(0), A.size(1), A.size(2))
        if A.dim() != 5:
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name2 + "_sigma_sq.h5", self.file2_key)).to(self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name1 + "_" + name2 + "_sigma12.h5", self.file1_key)).to(
            self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
        ...
        A = torch.from_numpy(Metric.load_file(dir_path + "/" + name1 + "_mu_sq.h5", self.file1_key)).to(self.device)
        if A.dim() == 3:
            A = A.expand(1, 1, A.size(0), A.size(1), A.size(2))
        if A.dim() != 5:
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name2 + "_mu_sq.h5", self.file2_key)).to(self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name1 + "_" + name2 + "_mu1_mu2.h5", self.file1_key)).to(
            self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
        ...
        B = torch.from_numpy(Metric.load_file(dir_path + "/" + name1 + "_" + name2 + "_cs_map.h5", self.file1_key)).to(
            self.device)
        if B.dim() == 3:
            B = B.expand(1, 1, B.size(0), B.size(1), B.size(2))
        if B.dim() != 5:
...
def main() -> None:
    ...
    parser.add_argument(
        "-k", "--k_constants", default=(0.01, 0.03), help="K constants"
    )
    parser.add_argument(
        "-d", "--device", default="cpu", help="Device where PyTorch will perform the calculations"
    )
    parser.add_argument('--overwrite', action='store_true')
    parser.add_argument('--no-overwrite', dest='overwrite', action='store_false')
    parser.set_defaults(overwrite=False)
    parser.add_argument(
        "-tmp", "--tmp_root", default=None, help="A prefix to be used when a temporary directory is created"
    )
